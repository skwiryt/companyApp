...
describe('Department', () => {
  ...
  describe('Updating data', () => {
    ...
    it('should properly update multiple documents with "updateMany" method', async () => {
      await Department.updateMany({}, { $set: { name: '=Department #1=' }});
      const updatedDepartments = await Department.find({ name: '=Department #1=' });
      expect(updatedDepartments.length).to.equal(2);
    });
  ...
  describe('Removing data', () => {
    ...
    it('should properly remove one document with "deleteOne" method', async () => {
      ...
      const departmentsLeft = await Department.find();
      expect(departmentsLeft.length).to.equal(1);
      expect(departmentsLeft[0].name).to.equal('Department #2');
    });
    ...
    it('should properly remove multiple documents with "deleteMany" method', async () => {
      await Department.deleteMany();
      const departmentsLeft = await Department.findOne();
      expect(departmentsLeft).to.be.null;
